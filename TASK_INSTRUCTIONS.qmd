...
The components are the building blocks of the task. They can be methods, metrics or control methods. Each component has a `config.vsh.yaml` file, a `script.py` or `script.R` file. The `config.vsh.yaml` file contains the information about the component, the `script.py` or `script.R` file contains the code for the component. The Openproblems team has provided a template for each component that already takes into account the required fields for the component depending on the API files.

To create the components you can create new scripts to the `scripts` directory with the following names: `add_a_control_method.sh`, `add_a_method.sh` and `add_a_metric.sh`.


```{.bash filename="scripts/add_a_control_method.sh"}
#!/bin/bash
